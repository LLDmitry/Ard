...
typedef struct {
  byte Command;               //1b  тип команды
  byte roomNumber; 	          //1b команда для комнаты №
  byte tOut;                  //2b внешняя темп
  byte tOutDec;                  //2b внешняя темп
  byte p_v;                   //2b атм давление
...
typedef struct {
  ...
  byte co2;                                                                               //2
  byte h;                                                                                     //1
  byte alarmType = ALR_NO;  //voda/motion alarm/...                                    //1    
} NRFResponse;  //18b
